...
btnAnterior.addEventListener("click",function (){
    if (paginaAtual > 0) {
        secoes[paginaAtual].style.display = "none"; // Oculta a seção atual
        paginaAtual--; // Volta para a pagina anterior
        secoes[paginaAtual].style.display = "block"; // Exibe a próxima seção   
    }
});
...
//---------------------------- VALIDAÇAO---------------------------------//
/*
 # CRIAR UM ID PARA O INPUT Q VAI SER VALIDADO
 # VINCULAR ESSE INPUT A UMA VARIAVEL JS
 # RECEBER O TEXTO DAQUELE ELEMENTO
 # BUSCAR A PRESENÇA DO CARACTERE ''ESPAÇO''
 # VERIFICAR SE ANTES E DEPOIS DO ESPAÇO TEM LETRAS
...
function validaNome(){
    ...
    let espaco = texto.index0f(' ')

    if(espaco != -1 && espaco!= 0 && espaco != (texto.length-1)){
        console.log("Achou  o espaço")
    } 
    /* poderia usar expresao regular é simples mas nao facil de determina la*/
};
